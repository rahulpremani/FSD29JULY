...
public class withdrawMoney extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		CustomerDataHandlerClass dataHandler = (CustomerDataHandlerClass) request.getServletContext()
				.getAttribute("dao");
		...
		if (dataHandler.withdrawMoney(request.getSession().getAttribute("mobileNumber").toString(), amount)) {
			float finalAmount = dataHandler.showBalance(request.getSession().getAttribute("mobileNumber").toString());
			response.getWriter().write("<script>alert('Amount withdrew successfully. Your current balance is :"+finalAmount+"');</script>");
